...
pub fn run() {
    let input = fs::read_to_string("src/resources/input02").expect("Should have been able to read the file");
    println!("{}", part1(&input));
    println!("{}", part2(&input));
}
...
#[test]
fn test() {
    let input = fs::read_to_string("src/resources/test02").expect("Should have been able to read the file");
